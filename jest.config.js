...
module.exports = {
	...
	moduleNameMapper: {
		...
		// will tell jest where to find source code for @konomi-network/ packages, it points to the src instead of dist.
		...packages.reduce(
			(acc, name) => ({
				...acc,
				[`@konomi-network/${name}(.*)$`]: `<rootDir>/packages/./${name}/src/$1`,
			}),
			{}
		),
	},
