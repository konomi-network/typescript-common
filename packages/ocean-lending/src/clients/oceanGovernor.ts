...
import { PoolConfig, TokenConfig } from "config";
...
export class OceanGovernor extends Client {
    /**
     * Propose a new currency in the OceanLendingOracle
     * @param address The address of proposed token
     * @param tokenConfig The configuration of proposed token
     */
    public async propose(address: string, tokenConfig: TokenConfig): Promise<BigInt> {
        const calldatas = OceanEncoder.encodeSingle(tokenConfig);
        const proposalId = await this.contract.methods.propose(address, calldatas).call();
        return proposalId;
    }

    /**
     * Propose  mutil new currencies in the OceanLendingOracle
     * @param addresses The addresses of proposed tokens
     * @param params The configurations of proposed tokens
     */
    public async proposeMulti(addresses: string[], params: PoolConfig): Promise<BigInt> {
        const calldatas = OceanEncoder.encode(params);
        const proposalId = await this.contract.methods.proposeMulti(addresses, calldatas).call();
        return proposalId;
    }
