...
async function getBorrowRate(jumpInterestV2: JumpInterestV2, oToken: OToken) {
  ...
  console.log(
    `cash: ${cash}, totalBorrows: ${totalBorrows}, totalReserves: ${totalReserves}, borrowRate: ${rate.valueOf() / ONE_ETHER
    }%`
  );
  console.log('==== getBorrowRate ====');
...
async function getSupplyRate(jumpInterestV2: JumpInterestV2, oToken: OToken, reserveFactorMantissa: BigInt) {
  ...
  console.log(
    `cash: ${cash}, totalBorrows: ${totalBorrows}, totalReserves: ${totalReserves}, reserveFactorMantissa: ${reserveFactorMantissa} supplyRate: ${rate.valueOf() / ONE_ETHER
    }%`
  );
  console.log('==== getSupplyRate ====');
  expect(totalReserves).to.be.eq('0');
...
async function getBorrowRateAPY(jumpInterestV2: JumpInterestV2, oToken: OToken, blockTime: number) {
  ...
  console.log(
    `cash: ${cash}, totalBorrows: ${totalBorrows}, totalReserves: ${totalReserves}, BorrowRateAPY: ${rate.valueOf() / ONE_ETHER
    }%`
  );
  console.log('==== getBorrowRateAPY ====');
  expect(totalReserves).to.be.eq('0');
...
async function getSupplyRateAPY(jumpInterestV2: JumpInterestV2, oToken: OToken, blockTime: number) {
  ...
  console.log(
    `cash: ${cash}, totalBorrows: ${totalBorrows}, totalReserves: ${totalReserves}, supplyRateAPY: ${rate.valueOf() / ONE_ETHER
    }%`
  );
  console.log('==== getSupplyRateAPY ====');
  expect(totalReserves).to.be.eq('0');
