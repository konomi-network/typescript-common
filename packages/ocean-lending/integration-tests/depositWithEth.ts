...
describe('DepositWithEth', async () => {
  ...
  before(async () => {
    ...
    oEth = new OToken(web3, oEthAbi, config.oTokens.oKono.address, account, config.oTokens.oKono.parameters);

    // load the erc20 token object
    ethToken = new ERC20Token(web3, ethAbi, oEth.parameters.underlying, account);
  });
