...
async function stakeOfTest(account: Account, token: ERC20Token, stakingV1: StakingV1) {
	...
	console.log(`erc20: ${erc20}, depositedAmount: ${depositedAmount}, totalReward: ${totalReward}`);
}

// Deposit some tokens
async function depositTest(account: Account, token: ERC20Token, stakingV1: StakingV1, amount: number) {
	console.log('==== deposit ====');
	...
	console.log('erc20Before:', erc20Before, ' depositedAmountBefore:', depositedAmountBefore, 'totalRewardsBefore', totalRewardsBefore);
	...
	console.log('erc20After:', erc20After, ' depositedAmountAfter:', depositedAmountAfter, 'totalRewardsAfter', totalRewardsAfter);
	...
	ensure(depositedAmountAfter > depositedAmountBefore, 'invalid deposit balance');
	console.log('==== deposit ====');
}

// WithDraw some tokens
async function withdrawTest(account: Account, token: ERC20Token, stakingV1: StakingV1, amount: number) {
	...
	const expectedDepositedAmount = depositedAmountBefore.valueOf() - withdrawAmount;
	ensure(depositedAmountAfter == expectedDepositedAmount, `invalid depositedAmount, expected ${expectedDepositedAmount}, actual: ${depositedAmountAfter}`);
	ensure(depositedAmountAfter <= depositedAmountBefore, 'invalid deposit balance');
	console.log('==== withdraw ====');
}

// WithDraw all tokens
async function withdrawAllTest(account: Account, token: ERC20Token, stakingV1: StakingV1) {
	...
	ensure(depositedAmountAfter.valueOf() == BigInt(0) && totalRewardsAfter.valueOf() == BigInt(0), `invalid deposit balance,  expected: 0, actual: ${depositedAmountAfter},  expected: 0, actual: ${totalRewardsAfter}`);
	console.log('==== withdrawAll ====');
}
