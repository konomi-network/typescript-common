...
export async function stakeOf(
  ...
  logger.info(
    "erc20: %o, depositedAmountBefore: %o, totalRewardsBefore: %o",
    erc20Before,
    depositedAmount,
    totalReward
  );
...
export async function deposit(
  ...
  logger.info(
    "erc20Before: %o, depositedAmountBefore: %o, totalRewardsBefore: %o",
    erc20Before,
  ...
  await stakingV1.deposit(depositAmount.toString(), confirmations);
  ...
  logger.info(
    "erc20After: %o, depositedAmountAfter: %o, totalRewardsAfter: %o",
    erc20After,
...
export async function withdraw(
  ...
  await stakingV1.withdraw(withdrawAmount.toString(), confirmations);
...
export async function withdrawAll(
  ...
  await stakingV1.withdrawAll(confirmations);
...
export async function parseStakingConfiguration(options: OptionValues) {
  ...
  return [account, token, stakingV1, config] as const;
}
...
export function makeStakingCommand(): Command {
  ...
  staking
    ...
    .requiredOption(
      "-c, --config-path <path>",
      "staking configuration json path"
    )
    .option(
      "-o, --oToken <path>",
      "staking oToken configuration json path",
      "../ocean-lending/config/oToken.json"
    )
    .option(
      "-e, --erc20 <path>",
      "staking erc20 configuration json path",
      "../ocean-lending/config/erc20.json"
    )
    .option(
      "-s, --stakingV1 <path>",
      "staking configuration json path",
      "../ocean-lending/config/stakingV1.json"
    );
  ...
  staking
    ...
    .action(async (options: OptionValues) => {
      logger.info(
        "Staking: stakes started, configPath: %o",
        staking.opts().configPath
      );
      const [account, token, stakingV1, config] =
        await parseStakingConfiguration(staking.opts());
  ...
  staking
    ...
    .action(async (options: OptionValues) => {
      logger.info(
        "Staking: deposit started, configPath: %o, amount: %o ",
        staking.opts().configPath,
        options.amount
      );
      const [account, token, stakingV1, config] =
  ...
  staking
    ...
    .action(async (options: OptionValues) => {
      logger.info(
        "Staking: withdraw started, configPath: %o, amount: %o",
        staking.opts().configPath,
        options.amount
      );
      const [account, token, stakingV1, config] =
  ...
  staking
    .command("withdrawAll")
    .description(
      "withdraw all deposit and reward of user stake into the contract."
    )
    .action(async (options: OptionValues) => {
      logger.info(
        "Staking: withdrawAll started, configPath: %o",
        staking.opts().configPath
      );
      const [account, token, stakingV1, config] =
        await parseStakingConfiguration(staking.opts());
      await withdrawAll(account, token, stakingV1, config.clients.txnOptions);
